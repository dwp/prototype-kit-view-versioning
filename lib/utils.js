...
// Use versioned view if available, if not carry on to default version
exports.redirectToVersion = function (req, res, next) {
  const verNum = req.session.data.v
  // If a version number is specified, try going to the version specific view
  if (verNum) {
    const path = req.path.replace('default', 'v' + verNum)
    const renderedPath = path.substr(1) // strip '/' so path can be rendered
    res.render(renderedPath, {}, function (error) {
      if (!error) {
        // If the version specific exists, go to that
        return res.redirect(path)
      } else {
        // If the version specific view doesn't exist, carry on
        return next()
      }
    })
  } else {
    // If no version number is specified, carry on
    return next()
  }
}
